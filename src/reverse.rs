...
fn reverse_scored(select: u32) -> Option<u32> {
    match select {
        1 => Some(5),
        2 => Some(4),
        3 => Some(3),
...
fn is_reversed_120(x: u32, y: u32) -> u32 {
    if IPIP_NEO_ITEMS_REVERSED_120.contains(&x) {
        reverse_scored(y).unwrap_or(y)
    } else {
        y
    }
}

pub fn reverse_scored_120(answers: Vec<HashMap<String, u32>>) -> Result<Vec<HashMap<String, u32>>, &'static str> {
